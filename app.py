...
with tabs[3]:
    ...
    with col2:
        ...
        churn_table = merged.groupby('churn', observed=False).size().reset_index(name='count')
...
with tabs[4]:
    ...
    with col2:
        ...
        summary = merged.groupby('churn', observed=False).agg({'customer_id':'count', 'monthly_expenditures':'mean', 'salary':'mean'}).reset_index()
...
with tabs[5]:
    ...
    band_summary = bands.groupby('salary_band', observed=False).agg(avg_salary=('salary','mean'), avg_expenditure=('monthly_expenditures','mean'), count=('customer_id','count')).reset_index()
